...
Next: run the smallest real probe for "20+ step BITWISE_PASS reproduction achieved", store its command output as an artifact, then call repro({ action: "record", requirementId: "bitwise-pass-20", proof: { kind: "validation", command: "...", resultRef: "artifact:...", passed: true } }).
