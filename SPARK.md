...
- `packages/spark-orchestrator`：ready task frontier 调度、dispatch-time role assignment、workflow-run 状态与 reconciliation。
- `packages/spark-goal`：vendored Spark-owned goal continuation primitives，用于 `/workflow:goal` / goal strategy 的 hidden-continuation prompt。
- `packages/spark-workflows`：vendored Spark-owned workflow runtime、最小 builtin workflow script contract、示例 workflow factories 与 role-run adapter 边界。
- `packages/spark-artifacts`：typed artifact store、hash/blob/provenance/lineage。
- `packages/spark-ask`：Spark ask artifact persistence/replay/tool helpers，建立在 `pi-ask` 和 artifacts 之上；具体 ask 问题由调用处基于实际上下文生成，不提供制式表单。
...
- `spark_status` 和 Spark widget 能以低噪声方式展示当前 project、active tasks、TODO、Spark workflow-run 状态和 readiness 问题。
...
- 检查外部 workspace 和历史文档里是否还残留旧 role 命名字段；持久化快照已经不接受 legacy `agentRef` / `agentName` / `subagent` 形状。
