...
Runner 和 Workspace 在产品概念上是一组关系：Runner 是用来创建或注册
Workspace 的入口，Workspace 是实际承载项目、资产和运行状态的工作区。

创建 Runner 表示在某台机器上注册一个可用工作路径。一个机器可以注册多个
Runner，但每个 Runner 使用的路径必须不同。创建 Workspace 时选择一个 Runner，
...
- Runner 负责把某台机器上的某个路径注册为可创建 Workspace 的运行位置。
- Workspace 负责运行时状态、项目、工作副本、证据和项目级资产。
- 同一台机器允许多个 Runner 注册，但路径不能重叠。
- Workspace 创建时从 Profile 复制或引用声明资产，然后进入运行时管理。
- 后续用户主要面对 Workspace，而不是 Runner。
...
- **Runner / Workspace 专题**：Runner 注册、路径互斥、Workspace 创建、
...
```text
1. 用户在机器上注册 Runner，得到一个可创建 Workspace 的路径。
2. 用户创建 Workspace，选择 Profile 或 fresh，并选择 Runner。
