# RFC: Navia projection store boundary inside Spark
...
After the Spark merge, Navia's SQLite database is a local web-cockpit
projection/cache. It is not the authoritative task, run, artifact, ask, or
review store for Spark execution. Spark `.spark/` stores remain the execution
source of truth; Navia stores enough mirrored state to render the cockpit,
route runtime protocol messages, reconnect safely, and cache artifact previews.

## Source-of-truth matrix

| Area | Authoritative owner | Navia SQLite role | Replay/idempotency key |
| --- | --- | --- | --- |
...
| Human asks/reviews/responses | Spark ask/review/task flows, surfaced over Navia protocol | Inbox projection and delivery tracking in `human_requests`, `human_responses`, `inbox_items`, `asks`, and `reviews`; Navia can record local user response delivery state | Runtime message receipt plus `human_requests(runtime_workspace_binding_id, runtime_request_id)` |
| Audit/event feed | Navia server local audit | Append-oriented diagnostic log, not execution truth | Runtime message receipt prevents protocol replay from duplicating ingested message effects |

## Rules

1. Navia must call Spark APIs for execution and durable Spark task/artifact
...
3. Snapshot and artifact projection functions remain idempotent even when called
   ...
   and replace links.
4. Navia `projects` rows are cockpit routing/grouping records, not Spark task
...
5. Navia tables may denormalize Spark refs into JSON payloads or content refs for
