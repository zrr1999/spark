...
This RFC freezes the remaining implementation-level choices before code starts. The goal is to make the first development pass boring: scaffold one SvelteKit app, one separate `apps/runner` process, a small set of high-cohesion packages, SQLite-backed server/runner stores, one protocol package, and enough test fixtures to validate the runner boundary.

No further product decisions are required before starting the first code slice. Future hosted/team runner implementation questions are explicitly deferred unless noted below.
...
| I02 | Code terminology          | Use **workspace registration** and **workspace directory** for product/operator-facing setup flows. Use **runner** when naming the `navia-runner` CLI/package or local daemon implementation boundary. Keep **runtime** only for existing protocol/API route/database/wire-contract identifiers such as `/api/v1/runtime/*`, `runtime.hello`, and `runtime_workspace_bindings`. |
...
| I06 | Runner WebSocket support  | Attach `ws` to a custom Node server around the SvelteKit handler; do not try to model the runner protocol WS as a normal SvelteKit route handler.                                                                                                                                                                                                                               |
| I07 | Frontend realtime         | SSE for browser event streams via `/api/v1/events`; runner control remains WebSocket-only.                                                                                                                                                                                                                                                                                      |
...
| I13 | Protocol versioning       | Runner protocol string is `navia.runtime.v1alpha1`; HTTP path is `/api/v1/runtime/*`; pre-v1 unsupported versions fail loudly.                                                                                                                                                                                                                                                  |
| I14 | Database file             | Server DB defaults to `${XDG_DATA_HOME:-~/.local/share}/navia/server/navia.sqlite`; runner DB defaults to `${XDG_DATA_HOME:-~/.local/share}/navia/runner/runner.sqlite`. Explicit env overrides may point tests/dev smoke at repo-local paths.                                                                                                                                  |
...
| I18 | Migrations                | Explicit SQL migrations in `packages/db/src/migrations`; small TS runner records applied migrations.                                                                                                                                                                                                                                                                            |
...
| I22 | Projection persistence    | Persist raw snapshots/events and normalized query tables for latest UI projections. Do not fetch project/task state on demand from the runner for normal page loads.                                                                                                                                                                                                            |
...
| I27 | Runner auth               | One-time enrollment token -> hashed runner token. Store only token hashes; support revoke/rotate.                                                                                                                                                                                                                                                                               |
| I28 | Runner heartbeat          | Runner sends heartbeat every 15s; server marks stale/offline after 45s or closed WS, but never auto-fails invocations from liveness alone.                                                                                                                                                                                                                                      |
| I29 | Command delivery          | Server-owned delivery state with idempotency keys; commands route only to the workspace's owning runner binding.                                                                                                                                                                                                                                                                |
| I30 | Human requests            | Runner-originated, idempotent by `(runtime_binding_id, runtime_request_id)`, wait indefinitely, and redeliver answered-but-unacked responses after reconnect.                                                                                                                                                                                                                   |
| I31 | Reminders                 | Store `next_reminder_at`; v0.1 shows badges/visible reminders. No email/push and no automatic timeout.                                                                                                                                                                                                                                                                          |
| I32 | Artifact cache            | Server lazy cache under `${XDG_CACHE_HOME:-~/.cache}/navia/server/artifacts`, content-addressed where possible, preview-first then unpinned LRU/TTL/size-cap eviction. Runner canonical artifacts live under runner XDG data.                                                                                                                                                   |
...
6. Runner enrollment tokens are created by the owner and exchanged once for runner tokens.
7. Runner tokens are hashed at rest and scoped to runner identity/workspace grants.
...
Normal UI page loads query SQLite projections. Runner snapshots/events update those projections. The server may request reconciliation, but it does not fetch task graph state from the runner on every page load.
...
3. `apps/web` SvelteKit app shell with adapter-node and custom server placeholder for runner WS.
...
6. Runner registration endpoint and token exchange.
7. Runner WS hello/heartbeat against a protocol test client.
8. Workspace overview page backed by SQLite.
9. `pnpm check` and `pnpm test` passing.

Do not start with the full UI, full task graph renderer, or real runner provider execution. Those depend on the core protocol/data spine.

## Deferred implementation details

The following remain intentionally out of v0.1 implementation, even though schema/protocol leaves room for them:

- direct browser-to-runner diagnostic/pairing;
