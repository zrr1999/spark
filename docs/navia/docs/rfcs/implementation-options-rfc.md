...
| I02 | Code terminology          | Use **workspace registration** and **workspace directory** for product/operator-facing setup flows. Use **Spark daemon** when naming the `spark-daemon` CLI/package or local daemon implementation boundary. Keep **runtime** only for existing protocol/API route/database/wire-contract identifiers such as `/api/v1/runtime/*`, `runtime.hello`, and `runtime_workspace_bindings`. |
