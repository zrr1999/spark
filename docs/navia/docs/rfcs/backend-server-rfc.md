...
- Implement multi-provider runner abstraction beyond the Spark runtime bridge.
- Choose additional runner implementation languages.
- Define local workdir/checkout cleanup internals.
- Make browser-to-runner direct connection the default path.
...
apps/navia-web (SvelteKit UI + server routes)
  ...
  +-> runner protocol endpoints/channels
```
...
- Connection health/diagnostics views, with runner identity downplayed.
...
- Runner heartbeat/liveness/diagnostics.
- Runner log/event/artifact projection ingestion.
- Runner protocol docs and fixtures.

### Runner responsibilities

- Authentication/config UX for the runner.
...
- Runner-side cancellation, cleanup, and safety policy.

## User-to-runner connectivity

Default v0.1 rule: **browser/user actions go through Navia's SvelteKit server, not directly to the runner**.
...
Migration runner can be a small TypeScript script. Keep migrations reviewable and reversible where reasonable.
...
- Cache eviction must never delete canonical external/runner artifacts; it only removes Navia's local cached copy or preview.
...
| -------------------------------- | ----------------------------------- | ------------------------------------------------ |
| Runner control/session           | WebSocket                           | bidirectional messages and lower-latency control |
...
| `task-graphs`         | runner-owned task graph projections/snapshots for cockpit/status            |
...
| `invocations`         | runner-reported invocation/run projections and event mirrors                |
...
This repo owns protocol and server communication endpoints; `apps/navia-web` must not import runner internals.
...
- Frontend should remain workspace/project-first and downplay runner as connection diagnostics rather than primary product navigation.
...
- Each server-visible workspace has exactly one active owning runner workspace binding in v0.1.
...
- Human requests/responses live in server DB as delivery/inbox records, but agent-facing tool contracts remain runner-owned.
- Canonical runner-produced artifact content remains external; server artifact metadata/projections and lazy cache pointers live in DB.
...
- Mirrored invocation history is durable evidence/projection; runner-owned retries create/report new invocations rather than rewriting old terminal attempts.
...
- Runner clients authenticate with tokens or device credentials.
- Tokens are scoped to runner identity plus workspace grants/capability policy.
...
- Runner bridge decides whether/how the routed command executes through Spark runtime APIs and enforces execution safety/provenance.
- Browser-to-runner direct connection is not a default path; v0.1 only keeps a design placeholder for a future explicit diagnostic/pairing channel.

Do not rely on runner self-reporting for product authorization decisions.
...
- runner registered/offline/draining;
...
- cache writes land under the XDG server artifact cache and eviction respects TTL/LRU/size caps without deleting canonical runner artifacts;
...
- Runner enrollment token exchanged for hashed runner credential.
...
Artifact cache eviction = metadata-driven LRU/TTL/size cap, never deletes canonical runner artifacts
Browser-to-runner direct connection = no product path by default; diagnostic/pairing placeholder only
Task graph = Spark-owned truth, Navia projection/snapshot UI
Repo scope = SvelteKit frontend/server + separate Spark-bridged runner process + protocol/contracts + SQLite projections + UI
