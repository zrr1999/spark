...
1. `node --version` reports `>=26 <27` and `pnpm --version` reports `>=11 <12`.
   The runner workspace pins both via `engines` and the daemon's `target` is
   `node26`.
...
- The web app has been changed to require a different `profileSource` enum
  value. Update both the cockpit form and `scripts/runner-smoke.ts`.
- The fake runner registered a workspace with a `displayName` and
...
The in-process runner registered against the server but never produced an
...
- Workspace path missing. The gate creates the temp workspace directory before
  ...
  to require a non-empty repo. Inspect the runner DB under the printed runner
  HOME (use `--keep-data`).

## `pnpm release:e2e` fails: "Timed out waiting for the runner to mark the invocation as succeeded"
...
- For `--real-pi`, raise `--task-timeout-ms` and confirm the runner data
  directory has Pi credentials. Run the daemon manually with
  `NAVIA_RUNNER_HOME=… pnpm runner start` and inspect the logs.
- For stubbed-Pi mode, check the daemon log under
  `<runner-state>/logs/runner.jsonl` for `daemon.error` outbox rows.
...
- `mirrored_invocations.runtime_invocation_id` is the id the runner generated,
...
Plain interactive `navia ws register` prompts on TTY if no path is provided.
...
```bash
navia ws register . --server-url https://… --token …
```
...
`apps/runner/src/cli.test.ts` cover this code path.
