...
Adopt a lightweight SvelteKit cockpit app plus a separate local service process inside the Spark TypeScript monorepo:
...
Goal: align docs around SvelteKit + SQLite + projections + same-monorepo runner process + runner/server protocol.

Docs to add/update before implementation:

- `backend-server-rfc.md` — done.
- `runner-protocol-rfc.md` — done for protocol boundary; protocol code keeps `runtime` wire terminology, while UI and operator-facing code use `runner`.
...
- Implementation options are frozen enough to start scaffolding: pnpm monorepo, Node 26, SvelteKit adapter-node, native `node:sqlite`, Kysely adapter, Zod protocol package, local owner setup, runner enrollment token flow, and WS runner control.
...
Goal: create the smallest runnable Navia codebase that proves tooling, protocol schemas, SQLite migrations, local setup auth, and runner registration/heartbeat can work together.
...
- `pnpm test` runs protocol schema tests and migration-from-empty tests. — green; includes Kysely dialect and runner WS handler tests.
...
- A runner smoke client can register, open WS, send hello/heartbeat, and update connection state. — done via `pnpm smoke:runner`; it also sends representative inbox/task/invocation/artifact projections and checks web/API paths.

Do not include real provider execution, direct browser-to-runner pairing, full graph rendering, or hosted/team auth in this slice.
...
- Runner processes register once and may report multiple workspace bindings.
- Workspace creation selects exactly one owning runner workspace binding in v0.1.
- Server routes commands/human responses to the workspace's owning binding and records delivery/ack/reject state.
- Runner owns execution feasibility, task graph truth, local orchestration, agent ask/review tool waits, and invocation lifecycle truth; server mirrors snapshots/events for frontend queries and subscriptions.
...
- Runner registration/liveness state.
- Workspace creation binds to exactly one owning runner binding.
...
- Connections/diagnostics (secondary; not primary runner navigation)
- Agent specs
- Resources
- Settings
...
- Runner identity is shown for diagnostics/provenance, not as the primary navigation object.
...
- runner workspace binding schema;
...
- runner-side safety policy.

Spark task graph, run, and artifact stores remain the canonical execution truth behind that bridge.

Tests:

- Runner registration and duplicate re-registration converge.
- A runner can report multiple workspace bindings.
- Workspace creation binds to exactly one owning runner binding.
...
- Runner-originated asks/decisions/approvals/reviews appear as inbox items.
- The agent layer only sees tools such as `ask_user`; runner converts the blocking tool call into a human request protocol message.
- Server/frontend collect the answer and deliver a human response back through the owning binding.
- Runner returns the response to the blocked tool call so the agent can continue.
...
- User-submitted pre-project request intake is deferred; if added later, it should be a separate intake flow, not confused with runner-originated asks/reviews.

Tests:

- Runner-created ask appears in web inbox.
...
Goal: make evidence first-class without eagerly copying runner content.
...
- Canonical runner-produced artifact content remains external.
- Server may cache/proxy content lazily on first frontend view/export.
- Artifacts can be produced by humans, asks, reviews, backend imports, or runner projections.
...
- Spark task/run stores own task graph mutation and execution truth; the Navia runner bridge reports projections.
- Navia stores snapshots/projections with provenance/version timestamps.
- Tasks may produce server commands routed to an owning runner workspace binding; the Spark bridge decides execution feasibility through Spark runtime APIs and reports invocation/run lifecycle.
...
- Server-side setup/diagnostic commands for runner registration tokens and workspace connection health.
...
- Runner setup flow is scriptable.
...
|     2 | Runner/browser protocol contracts | Slice 0                                     | Add Zod schemas and fixtures for workspace snapshots, commands, human requests/responses, task graph snapshots, invocation/log updates, and artifact projections.                                    |
...
|     4 | Runner ingestion/delivery channel | DB services                                 | Extend runner WS/API beyond hello/heartbeat to persist projections and deliver/ack/reject queued commands and human responses.                                                                       |
|     5 | Liveness + SSE                    | Runner ingestion                            | Mark stale runners/sessions and fan out useful append-only events to browser pages.                                                                                                                  |
|     6 | Workspace/project flows           | DB services + liveness                      | Done: workspace creation from runner bindings, project list/create/detail routes, cockpit projection summaries, and enabled Projects navigation.                                                     |
|     7 | Web ask inbox                     | Runner ingestion + projects                 | Done: `/inbox` lists runner-originated asks/reviews, persists user answers through projection services, exposes resolved/delivery state, and retries delivery to owner bindings without timeout.     |
