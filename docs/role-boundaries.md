...
- `packages/spark-workflows/src/orchestrator/index.ts` owns graph-level `/workflow:ready` scheduling: ready frontier scheduling, dispatch-time executor role assignment, workflow-run state, and stale run reconciliation.
...
- `spark-workflows` maps ready Spark task frontiers to scheduled `spark-runtime` runs for `/workflow:ready` and owns workflow-run scheduling/reconciliation state.
