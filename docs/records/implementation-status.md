...
- Daemon normalizes `assignment.create.request` onto the existing task execution path; optional channel ingress enqueues `session.run`
- Spec: [`assignment-and-channels.md`](../specs/assignment-and-channels.md)
- `spark-artifacts`
...
- `spark`
  ...
  - role-run registry/TUI integration for active, waiting, replied, failed, stale/interrupted, and completed background roles; reply/steer control attempts record provenance, delivery succeeds only when the active run exposes an input control channel, and visible `spark-role-runs` surfaces refresh after control attempts; validated against the role TUI audit (`artifact:5a554db7-6438-441f-b525-1f57ba4aef02`), closed the stop-refresh/failed-delivery needs-change review (`artifact:f43da8ee-bf94-41ea-ba72-bb162fa5e138`), and has approved control evidence (`artifact:223c907d-7034-4e18-8818-568c34ab03fa`, review `artifact:c00e9bed-c67d-42f4-90f0-410dad1bb06c`)
