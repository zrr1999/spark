...
Spark separates three data concepts behind two public tools and one shared execution engine:
...
- `packages/spark-session/src/*` owns the canonical `session({ action })` tool, persistent registry, local/channel classification, daemon RPC adapter, and mailbox mechanisms.
- `packages/spark-roles/src/extension.ts` exposes the canonical `role({ action })` tool for role definitions, model settings, and anonymous calls.
- Anonymous role calls and persistent session calls both enter the shared headless host and `SparkAgentSession`; the public split does not duplicate execution engines.
...
- A **RoleRun** is one concrete execution: run ID, launch mode, active control capability, stdout/stderr/events, timeout/cancel status, and optional fork source.
...
`spark-roles` owns reusable role definition data and anonymous single-run execution:
...
- Canonical Pi role tool `role({ action })`: role-definition management, model settings, and anonymous role calls.
...
- Persistent session registry/classification/calls/mail remain owned by `spark-session`; daemon execution remains behind local RPC.
