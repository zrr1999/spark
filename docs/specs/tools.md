...
- `role` — canonical role and session tool. A reusable role is a definition; `role({ action: "call", role, instruction })` runs it in an anonymous session. A managed session is persistent continuity; use `role({ action: "call", sessionId, instruction })` to submit a turn, and `action: "list" | "get" | "create"` with `resource: "session"` plus `bind | unbind | archive` for daemon registry lifecycle. `send | mailto | inbox | read | ack` provide durable local cross-session mail; `mailto` is a compatibility alias for `send`. The message envelope follows nyakore NNP's useful causal subset: the current session is always the sender, `kind` is `request | inform | reply`, `intent` and JSON `payload` are explicit, `correlationId` groups an exchange, and `replyToMessageId` routes back to the original request sender while inheriting correlation. Agent inbox access is current-session-only, sends are idempotent, and callers must not busy-wait. Unlike nyakore's dispatcher, Spark mail append never submits a turn or wakes the target; only an explicit persistent `call` executes a session. Role definition model settings remain under `model_list | model_get | model_set | model_delete`. Spark task execution should still prefer `assign({ dryRun: true })` so task claims, run records, and evidence attribution stay coherent.
...
- `role` — canonical role/session action tool. Role definitions and anonymous calls use `role`; persistent session lifecycle, calls, and mail use the same tool with `resource: "session"` or session-specific actions. Do not add fragmented role/session tool names.
...
- `role + instruction` starts an anonymous fresh session and does not retain conversation continuity;
- `sessionId + instruction` submits work to an existing persistent session through the daemon;
- `role` and `sessionId` are mutually exclusive call targets;
