...
- `/loop <focus>` — start or resume a persistent foreground loop for open-ended continuous work: monitoring fresh information, periodic repo checks, long-running observation, or continuing until the user stops it. `/loop` can drive repeated research/plan/implement steps and reports blockers, but it is not a `/goal` alias and must not call `goal({ action: "complete" })` or request reviewer-gated completion. Successful `/loop` turns choose their next cadence with `loop({ action: "schedule", delayMs, reason })`; there is no fixed default tick interval, and turns should call `ask` first when cadence/cost/urgency is a material user preference. `/loop stop` and Chinese stop aliases remove the plain loop instead of leaving a paused loop; `/loop pause` is removed. The TUI shows active loops and any scheduled cadence in the foreground widget slot as `◆ Loop(...)`.
...
- `role` — canonical reusable-role tool. `list | get | create` manage definitions, `model_list | model_get | model_set | model_delete` manage role model settings, and `role({ action: "call", role, instruction })` runs one fresh anonymous invocation. Persistent continuity is not accepted on this surface. Spark task execution should still prefer `assign({ dryRun: true })` when claims, run records, and evidence attribution are required.
- `session` — canonical persistent-session tool. `list | get | create | bind | unbind | archive` manage the daemon registry; list projections explicitly include `surface: "local" | "channel"`, `channelAdapters`, and `externalKeys`, with `surface`, `adapter`, `workspaceId`, `includeArchived`, `offset`, and `limit` filters. `call` submits a persistent turn through daemon `turn.submit`; it shares the same headless host and `SparkAgentSession` engine used by anonymous role calls. `send | mailto | inbox | read | ack` provide durable local peer mail; mail follows nyakore NNP's causal subset (`request | inform | reply`, `intent`, JSON `payload`, `correlationId`, `replyToMessageId`) and never submits a turn or wakes the target. Inbox access is current-session-only and sends are globally idempotent. Message-platform hosts activate only this tool and restrict it to `list | get | send | mailto | inbox | read | ack`: listing and targets remain inside the current workspace, get/send targets must be `surface=local`, and callers must not poll after forwarding work.
...
- `role` — canonical role-definition/model/anonymous-call action tool.
- `session` — canonical persistent lifecycle/classification/call/mail action tool.

These are separate public concepts over shared host/session execution mechanisms. Do not reintroduce `resource=session` on role or duplicate the headless execution engine. Role specs do not carry model policy: `model` and `defaultModel` frontmatter are rejected, and model choices are stored in `.spark/role-model-settings.json` or `~/.agents/role-model-settings.json`.
...
- `role({ action: "call", role, instruction })` starts an anonymous fresh session and does not retain conversation continuity;
- `session({ action: "call", sessionId, instruction })` submits work to an existing persistent session through the daemon;
- both paths reuse the same headless host and `SparkAgentSession` engine;
...
- `read` — read a text file with 1-indexed `offset`/`limit`. Output is truncated to 2000 lines or 50KB (whichever is hit first) with an actionable `Use offset=… to continue` notice.
- `write` — create or overwrite a file, creating parent directories.
- `edit` — exact-then-fuzzy multi-edit replacement; overlapping, duplicate, empty, and no-op edits are rejected with precise errors, and a line-numbered display diff plus unified patch are returned in `details`.
- `ls` — alphabetical directory listing with a `/` suffix for directories, capped at 500 entries / 50KB.
...
omitted because Spark uses `cue_exec` for shell execution.
