...
Inside an accepted invocation, transient provider request failures may retry without an attempt-count limit only while the abortable model-stream execution budget remains active. Cancellation or the execution deadline still stops them. A partially executed model/tool turn is never replayed as a unit because tool and provider side effects are not generally idempotent.
...
Every agent submission terminates with a `SparkRunOutcome`: `completed`, `aborted`, `budget_exhausted`, or `failed`. The legacy `submit()` API returns its assistant envelope, while `submitWithOutcome()` and headless callers consume the explicit terminal status. `roundtrips` counts attempted model calls, including a one-call answer without tools. Model-stream failures, aborts, cancelled approval waits, and roundtrip exhaustion therefore cannot be mistaken for completion.

Before each model roundtrip, the turn loop emits a privacy-safe `SparkPromptManifest`. It contains prompt version and hashes/character counts, a hashed session fingerprint, model/reasoning selection, cache-key fingerprint, active resolved tool policies, selected skill names, and remaining budgets. It never contains prompt text, user input, tool arguments, raw session IDs, credentials, or raw cache keys, and it is an observability event rather than a conversation message. The pure behavior evaluator scores recorded manifests/outcomes/tool summaries against explicit tool, effect, evidence, and budget expectations.
...
- Terminal run status is explicit; absence of another tool call is not sufficient evidence after an error, abort, or exhausted budget.
