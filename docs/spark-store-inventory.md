...
| `.spark/todos/todos.sqlite`                                                           | `pi-tasks` / `TaskTodoStore` SQLite compatibility/projection store                                                                | Canonical/protected session TODO records and task plan-item projection | `defaultTaskTodoStore(cwd, scope)`, `TaskTodoStore.load/save/hydrate()`, `loadSessionTodos()`, `saveSessionTodos()`; facades `task_write({ action: "todo_update" })`             | Hard-cutover V2 task progress truth lives in `TaskPlan.items`; this store persists session TODOs and imports/projects task plan items. Protected; not a cache.                         |
...
- `spark` owns per-session directories under `sessions/`, rebuildable `sessions/index.json`, session TODO facade calls over the `pi-tasks` SQLite TODO record store, roadmap planning helpers over per-project roadmaps in `.spark/projects/<project>/roadmap.json`, subject-owned review record writers, and housekeeping summaries that classify cache vs protected state.
