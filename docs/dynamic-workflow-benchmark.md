...
- `/workflows` is the control plane: progress by stage/agent, pause/resume/stop/restart, inspect agent details, and save a run as a reusable command.
...
4. **Resource bounds**: default 16-way concurrency, 1000-agent cap, optional run/stage token budgets, optional `maxAgents` and `concurrency` controls.
...
| Background/control UI for dynamic runs | Implemented, tested, documented; improved after demo feedback that raw tool output was below Claude Code's UX bar | `workflow_run` completion card plus `/workflows` and `task_read run_status` timeline/controls rendering tests |
...
- `task_read({ action: "run_status" })` / the `/workflows` dashboard now renders persisted dynamic `workflow_run` records from v2 snapshots with stage/fan-out/helper/nested trees, event tails, available controls, agent journal tail metadata/result snippets, background result inbox entries, completed results/errors, base metadata, actual/estimated token totals, optional cost, agent liveness/rate telemetry, Graft provenance, saved workflow selectors, acknowledged state, and next actions. Dynamic controls `pause`, `resume`, `stop`, `restart`, `save`, and `ack` route through the dynamic workflow manager and v2 event store; `restart` aborts the active generation and starts a fresh remembered run, `save` writes the script to controlled workspace or user workflow files with collision-safe suffixing, and `ack` hides delivered terminal runs from compact status.
