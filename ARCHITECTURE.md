...
- `spark-orchestrator` may combine `spark-tasks`, `spark-runtime`, `pi-roles`, and `spark-artifacts` to schedule ready task frontiers, assign executor roles at dispatch, and own workflow-run state.
...
- Role execution/runtime workflow scheduling is split: generic single-run launch/control/JSONL helpers live in `pi-roles`; Spark single-task adaptation lives in `spark-runtime`; graph-level ready frontier scheduling lives in `spark-orchestrator`.
