...
export const load: PageServerLoad = ({ cookies, url }) => {
  ...
  const page = loadWorkbenchHome(getDatabase(), { forceWorkspaceCreate, pendingWorkspaceSetup });
  if (page.redirectWorkspace) {
    redirect(303, workspacePath(page.redirectWorkspace));
  }
  return {
