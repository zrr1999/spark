<script lang="ts">
  ...
  import type { ProjectChatContextCard, ProjectChatPromptSuggestion } from "$lib/project-chat-context-actions";
  ...
  let {
    ...
    suggestions: ProjectChatPromptSuggestion[];
    contextCards: ProjectChatContextCard[];
  } = $props();
  ...
  function handleSuggestedSubmit(event: SubmitEvent, suggestion: ProjectChatPromptSuggestion) {
    if (!canStartTask) {
      event.preventDefault();
...
<div class="suggested-prompts" aria-label={t.suggestionsLabel}>
  {#each suggestions as suggestion}
    <form method="POST" action="?/startTask" onsubmit={(event) => handleSuggestedSubmit(event, suggestion)}>
      <input type="hidden" name="prompt" value={suggestion.prompt} />
      <button type="submit" disabled={!canStartTask && hasActiveRun} title={suggestion.meta}>
        {hasActiveRun ? t.queue : suggestion.label}
      </button>
    </form>
  {/each}
</div>
...
<form method="POST" action="?/startTask" class="chat-composer" onsubmit={handleComposerSubmit}>
  <label for="main-session-prompt">{editingQueuedId ? t.editQueued : t.messageLabel}</label>
  <textarea
  ...
  <div class="composer-footer">
    <div class="composer-controls" aria-label={t.contextLabel}>
      <span class="context-chip active"><Icon name="folder" size={14} />{t.projectContext}: {projectName}</span>
    ...
    <div class="submit-controls">
      {#if hasActiveRun}
        <button
          type="submit"
          form="main-session-cancel-run-form"
          class="stop-button"
...
<form id="main-session-cancel-run-form" method="POST" action="?/cancelRun" class="stop-form">
