/**
 * SparkHostRuntime — the native spark-cli implementation of the
...
export class SparkHostRuntime implements ExtensionAPI {
  ...
  registerTool = (config: ToolConfig): void => {
    ...
    for (const listener of this.toolRegistrationListeners) {
      try {
  ...
  listMessageRenderers(): Array<{ customType: string; renderer: SparkHostMessageRenderer }> {
    return [...this.messageRenderers.entries()].map(([customType, renderer]) => ({
      customType,
      renderer,
    }));
  }
  ...
  registerShortcut(
    ...
    this.keybindings.register({
      ...
      isActive: options.isActive ? (ctx) => options.isActive!(ctx as ExtensionContext) : undefined,
    });
  }
