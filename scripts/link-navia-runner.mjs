...
const cliPath = path.join(repoRoot, "apps/navia-runner/dist/cli.js");

if (!existsSync(cliPath)) {
  console.error(`Navia runner CLI is not built: ${cliPath}`);
  console.error("Run `pnpm run navia:install` to build and link the CLI.");
  process.exit(1);
}
...
const linkPath = path.join(binDir, "navia");
...
if (existsSync(linkPath)) {
  ...
  if (target !== cliPath) {
    console.error(`Replacing existing navia symlink: ${linkPath} -> ${target}`);
  }
  rmSync(linkPath);
}
...
console.log(`Linked navia -> ${cliPath}`);
console.log(`Global bin: ${binDir}`);
console.log(`If your shell cannot find navia, add this directory to PATH.`);
