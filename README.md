...
- `@zendev-lab/spark-extension-api` — shared extension host/tool contract, refs, errors, and light JSON/fs/time helpers. It remains separate from `spark-extension` so Pi-style host contracts stay host-neutral.
- `@zendev-lab/spark-artifacts` — reusable artifact/evidence store, durable artifact metadata/blobs, provenance/lineage contracts, and the canonical `artifact` action tool.
