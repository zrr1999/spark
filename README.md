...
The extension packages depend on the shared `@zendev-lab/pi-extension-api` contract, not on Pi's concrete SDK package. Shared host runtime code belongs in `packages/spark-host` and shared turn orchestration belongs in `packages/spark-turn`; `apps/spark-tui/src/host/` keeps compatibility adapters plus app bootstrap/session glue, and `apps/spark-tui/src/tui/` keeps TUI wrappers. The Pi extension implementation should stay usable by Pi without importing Spark app packages.
...
Pi package loading is manifest-first: the root `pi` manifest explicitly lists each user-visible extension entry (`@zendev-lab/pi-ask`, `@zendev-lab/pi-artifacts`, `@zendev-lab/pi-btw`, `@zendev-lab/pi-cue`, `@zendev-lab/pi-files`, `@zendev-lab/pi-roles`, `@zendev-lab/pi-recall`, `@zendev-lab/pi-workflows`, `@zendev-lab/pi-graft`, the Spark AI models extension, the Baidu OneAPI provider, and `@zendev-lab/spark-extension`). Library-only packages stay as dependencies. `pi-* -> spark-*` regressions are guarded by the boundary checker inside `pnpm run check`, a `prek` hook, and the CI static-check workflow, with `@zendev-lab/spark-tui` kept as the explicit shared UI-boundary exception.
