    ...
    try {
      ...
      messages.push(
        ...ctx.sessionManager.getEntries().flatMap((entry) => {
          ...
          return isVisibleBtwMessage({ role: message.role, customType: message.customType })
            ? []
            : [message as Message];
        }),
      );
...
function ensureTranscriptTurn(state: BtwTranscriptState): number {
  ...
  appendTranscriptEntry(state, { type: "turn-boundary", turnId, phase: "start" } as Omit<
    Extract<BtwTranscriptEntry, { type: "turn-boundary" }>,
    "id"
  >);
  return turnId;
}
  ...
  if (!content) {
    if (typeof value === "string") {
      ...
      try {
        ...
        content = "[unserializable tool output]";
      }
    }
  }
