...
import { createId, runtimeProtocolVersion, serverCommandEnvelopeSchema } from "@navia-dev/protocol";
...
interface TestHarness {
  ...
  db: ReturnType<typeof openRunnerDatabase>;
  workspace: ReturnType<typeof addWorkspace>;
  cleanup(): void;
}

function makeHarness(): TestHarness {
  const root = mkdtempSync(join(tmpdir(), "navia-runner-daemon-"));
...
function makeContext(
  ...
  const config: RunnerConfig = {
    installationId: "navia-runner-test",
    displayName: "Navia runner test",
  };
  return {
    paths: harness.paths,
    config,
...
describe("runner handleCommand task.start.request", () => {
  ...
  it("stops an idle daemon loop when shutdown is requested", async () => {
    ...
    try {
      ...
      const running = startRunnerDaemon({
        paths: harness.paths,
        db: harness.db,
        config: {
          installationId: "install-test",
