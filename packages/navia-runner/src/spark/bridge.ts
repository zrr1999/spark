...
import type { RunnerWorkspace } from "../store/workspaces.js";
import {
  artifactProjected,
  commandAck,
  commandReject,
...
export async function runNaviaCommandThroughSpark(
  ...
  try {
    ...
    const run = await executeSparkTask({
      ...
      claim: {
        kind: "role-run",
        sessionId: `navia:${input.route.runtimeId}`,
    ...
    const task =
      ...
      graph.createTask({
        ...
        plan: {
          ...
          contextRefs: [],
          constraints: ["Execute from Navia runner bridge; preserve Navia protocol projections."],
          ...
          steps: [
            "Run the requested task through Spark runtime.",
            "Report terminal status and evidence.",
          ],
          openQuestions: [],
          askRefs: [],
        },
  ...
  return commandReject(
    {
      ...
      message: "Runner has no active Spark invocation matching this cancellation request.",
      retryable: false,
    },
