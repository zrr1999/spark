...
import { runnerSupportedFeatures, runnerVersion } from "./daemon.js";
import { refreshRunnerCredentials, shouldRefreshRunnerToken } from "./token-refresh.js";
...
export interface LocalServiceRegistrationResult {
  config: RunnerConfig;
  workspaceBinding?: RuntimeRegistrationResponse["workspaceBinding"];
}

export async function registerLocalServiceWithToken(
  paths: NaviaPaths,
  input: LocalServiceRegistrationInput & { registrationToken: string },
) {
  const current = readRunnerConfig(paths);
  ...
  const registered = await requestLocalServiceRegistration({
    serverUrl,
...
export async function ensureLocalServiceRegistrationForWorkspace(
  paths: NaviaPaths,
  input: LocalServiceRegistrationInput,
): Promise<LocalServiceRegistrationResult> {
  let current = readRunnerConfig(paths);
  const serverUrl = validateRegistrationServerUrl(input.serverUrl);
  if (hasRunnableRunnerCredentialsForServer(current, serverUrl)) {
    current = shouldRefreshRunnerToken(current)
      ? await refreshRunnerCredentials({ paths, config: current })
      : current;
  ...
  const registered = await registerLocalServiceWithToken(paths, {
    ...input,
    serverUrl,
    registrationToken: input.registrationToken,
  });
...
export function hasRunnableRunnerCredentialsForServer(
  config: RunnerConfig,
  serverUrl: string,
): boolean {
  return Boolean(
...
export function configuredServerUrl(config: RunnerConfig): string | undefined {
  if (config.serverUrl) {
...
export async function verifyLocalServiceWorkspaceConnection(input: {
  config: RunnerConfig;
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      fail(
        new Error(
          `Workspace registration failed: local service could not confirm the server WebSocket within ${input.timeoutMs ?? 5_000}ms.`,
        ),
      );
...
async function requestLocalServiceRegistration(input: {
  serverUrl: string;
  registrationToken: string;
  displayName: string;
  installationId: string;
