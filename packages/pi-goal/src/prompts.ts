...
export function continuationPrompt(goal: Goal): string {
  return [
    ...
    "",
    `Do not call ${goalToolReference("complete")} unless the goal is complete. Do not mark a goal complete merely because the budget is nearly exhausted or because you are stopping work.`,
    "",
    GOAL_TOOL_NAME_GUIDANCE,
    "</spark_goal_continuation>",
  ].join("\n");
