...
`TaskGraph` records runs, readiness, dependencies, and TODO summaries, but does not execute agents. Runtime execution belongs in `spark-runtime`; agent registry/creation belongs in `spark-agents`. Dependencies are strictly scoped to tasks in the same thread; cross-thread/plugin dependency orchestration is intentionally out of scope for `spark-tasks`.
