...
Before this package, every extension package (`pi-ask`, `pi-cue`,
`pi-graft`, `pi-roles`, `spark`) maintained its own `pi-types.d.ts` shim
...
`pi-extension-api` collapses those copies into one source of truth. Each
...
- spark-cli native pi-tui host — `SparkHostRuntime` implements the retained
  surface needed by `pi-ask`, `pi-cue`, `pi-roles`, `pi-graft`, and `spark`,
...
| `spark-cli` SparkHostRuntime              | Retained extension surface implemented for native Spark CLI boot               |
...
5. If the change touches native Spark CLI boot/loading, also run the relevant
   `spark-cli` host tests (extension loader, runtime contract, and bootstrap).
