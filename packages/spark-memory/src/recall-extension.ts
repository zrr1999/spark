...
export function registerPiRecallTool(
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use recall only for explicit user/workspace/repo scoped recall candidates; do not silently write durable memory.",
      'Use memory({ action: "remember" }) for durable entries; use learning for evidence-backed knowledge artifacts.',
