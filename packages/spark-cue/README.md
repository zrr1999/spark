...
`@zendev-lab/spark-cue` honors cue-shell client transport resolution through `cue-client target resolve --json` or `cue client target resolve --json`.

- Unix profiles connect to the resolved daemon socket. If the local daemon is not reachable, `@zendev-lab/spark-cue` may auto-start `cued` for that Unix socket.
...
- Remote daemon startup remains explicit. `@zendev-lab/spark-cue` does not run `start_command` for SSH profiles; start the remote daemon yourself, for example `ssh user@example.com "cued start"`.
