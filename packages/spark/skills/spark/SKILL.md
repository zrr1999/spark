---
name: spark
description: Use for turning an initial or ambiguous project intent into a project/task DAG, artifacts, reviews, asks, and role plans through Spark's intent-specific commands and canonical task_read/task_write/assign/goal/workflow tools.
disable-model-invocation: true
---

# Spark

Spark extensions are dual-host: the same retained `pi-*` extension packages should run under Pi's `@earendil-works/pi-coding-agent` host and under the native `spark-cli` host via `SparkHostRuntime` / `pi-extension-api`. Keep shared extension behavior host-neutral; put native CLI boot, provider/model selection, sessions, skills, and pi-tui wrappers in `apps/spark/src/host/` or `apps/spark/src/tui/`.

Use Spark commands intentionally: lightweight research/investigation/reporting is the unconditional default behavior for ordinary input and auto-detects when project-bound state is needed; `/plan <focus>` refines the task DAG, `/implement <focus>` works through ready project tasks until the next blocker while blocking for human answers instead of auto-answering asks, `/loop <focus>` starts or resumes a persistent open-ended foreground driver for continuous monitoring, periodic checks, or progress until the user stops it (pause/stop commands mark it `paused` and it appears as `◆ Loop(...)`), and `/goal <focus>` drives autonomous verified foreground progress by layering goal objective tracking, reviewer-backed ask auto-answer/auto-decision, and reviewer-gated completion on top of loop primitives. `/goal` and `/loop` are mutually exclusive session foreground drivers. `/workflow[:selector] <focus>` runs builtin or saved Spark workflow scripts as an independent capability; it must not require initialized Spark project/task state or a selected project. Empty `/workflow` and `/workflows` open the blocking workflow navigator. Use `/workflow:research <question>` or `/workflow builtin:research <question>` for the builtin research workflow, and `/workflow:review <focus>` for skeptical review; use `/workflow workspace:<name>` and `/workflow user:<name>` for disk workflows. Do not reintroduce legacy run, execute command, `/research`, or virtual-model fusion guidance.
...
- `pi-loop`, `pi-goal`, and `pi-workflows`: reusable loop/goal/workflow primitives; Spark owns `/loop` continuation policy and project-bound `/goal` completion policy while `/workflow` remains a project-independent workflow capability.
...
18.   Prefer Spark-native delegation: inspect roles with `role({ action: "list" | "get" })`, bind concrete tasks to builtin/extension/project/user `roleRef`s, and hand execution to Spark workflow-run scheduling via `assign({ dryRun: true })`. For foreground implementation, `/implement` claims/finishes one concrete task at a time and continues until the ready frontier is empty or a human-blocking ask/review/blocker appears; it must not auto-answer asks. For autonomous foreground implementation, `/goal` uses Spark goal continuation prompts, may use reviewer-backed canonical ask auto-answer for material decisions, and continues verified task progress until done or blocked; when the user gives no focus, infer the objective from the active project/task state, and if reviewer auto-answer cannot resolve an ambiguity, report the blocker instead of waiting for raw human input. For scripted/subagent execution, use saved workspace/user workflows; workflow `agent()` calls must cross the Spark workflow role-run adapter boundary rather than raw Pi subagent spawning. Use `role({ action: "call" })` only for one-off direct role calls that should stay outside Spark tasks/DAGs.
