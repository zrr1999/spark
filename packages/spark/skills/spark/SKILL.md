...
14.   Prefer Spark-native delegation: inspect roles with `list_roles` / `get_role`, bind concrete tasks to builtin/project/user `roleRef`s, and hand execution to the `spark_run_ready_tasks` DAG manager. Use `call_role` only for one-off direct role calls that should stay outside Spark tasks/DAGs.
