...
- `spark-agents`: builtin/project agent specs; runtime execution is handled elsewhere.
...
4. Executable tasks must bind to a predefined or project agent spec.
...
13.   Before launching multiple agents or parallel workstreams, ask for approval with `spark_ask` unless the user explicitly requested immediate dispatch. Treat no-selection as blocked, not approval; asks do not support automatic timeout.
14.   Prefer Spark-native delegation: inspect specs with `spark_list_agent_specs` / `spark_get_agent_spec`, bind concrete tasks to predefined/project `agentRef`s, and hand execution to the `spark_run_ready_tasks` DAG manager. Legacy `spark_list_agents` / `spark_get_agent` aliases may exist, but use the spec-named tools in new instructions. Do not spawn nested `pi` CLI sessions as pseudo-agents unless the task is explicitly about testing Pi CLI behavior.
...
16.   Keep temporary plans, agent reports, and scratch outputs out of the repo root; use `.spark/notes/`, `.spark/agent-reports/`, or typed Spark artifacts unless the user asks for committed docs.
