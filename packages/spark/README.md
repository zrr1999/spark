...
- `@zendev-lab/pi-tasks` owns project/task/TODO graph state, readiness, optional role hints, claims, and the canonical `task` tool.
...
Patch/candidate workflows belong to `@zendev-lab/pi-graft`: use `graft_patch` for patcher-style child runs so the child receives only Graft-related tools and must escalate unclear instructions upward instead of editing directly.
