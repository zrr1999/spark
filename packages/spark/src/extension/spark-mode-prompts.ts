...
const SPARK_ASK_PLANNING_REMINDER = `Reminder for planning mode: do not guess user intent. Unless the user explicitly asks you to infer or research, if a user-facing open question or decision would change ${PLANNING_AFFECTING_CHOICES}, call ask with context-specific questions before task({ action: "plan" }); do not leave those questions as prose or plan.openQuestions. Prefer asking whether the user wants durable task planning, design options only, or execution toward completing all project tasks before narrowing the durable scope.`;

const SPARK_ASK_EXECUTION_REMINDER = `Reminder: do not guess user intent. Unless the user explicitly asks you to infer or research, if a missing user decision blocks execution or would change ${PLANNING_AFFECTING_CHOICES}, stop and call ask instead of inventing scope or finishing the task. In research/plan modes, prefer asking before converting findings into durable tasks or a narrower goal.`;

const SPARK_GOAL_NON_INTERACTIVE_REMINDER = `Reminder for goal mode: interactive ask tools are disabled, but do not guess user intent. Goal objectives should normally mean completing all unfinished work in the selected project, not stopping at a plan, unless the user explicitly says planning-only/readiness-only/仅规划. If task decomposition is wrong or missing, create or revise concrete tasks with task({ action: "plan" }); if a missing user decision would change ${PLANNING_AFFECTING_CHOICES} and cannot be inferred from context, stop, report the blocker, or pause the goal with goal({ action: "pause" }) instead of calling ask or inventing scope.`;
...
export function renderSparkResearchModePrompt(
  ...
  const action = selectedProjectRef
    ? 'Investigate the repository, current project, task graph, artifacts, context providers, and external references needed to answer the focus. Do not call task({ action: "plan" | "claim" | "finish" }) in research mode. When research changes task scope, suggests new work, or exposes multiple implementation directions, summarize findings and ask whether the user wants design options, durable task planning, or execution toward completing the project.'
