...
- `cue_jobs` — list, inspect, wait for, and stop jobs via `action`. List output is limited to 20 rows by default; chain status/wait output prioritizes failed/running/non-clean leaves and summarizes clean successful leaves.
