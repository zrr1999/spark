...
- `cue_exec` — execute commands and create cue-shell jobs. Tool/API runs use pipe mode (`pty: false`) by default; set `pty: true` only for commands that genuinely need terminal semantics. Foreground output is tailed to 16 KiB per stream by default (`tail_bytes: 0` for full output).
