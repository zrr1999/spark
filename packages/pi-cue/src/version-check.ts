/**
 * Detect outdated `cued` daemons from the pi-cue extension.
 *
 * pi-cue itself does not have an authoritative "expected" cued version —
 * it is shipped independently and may be installed against many cue-shell
 * releases over its lifetime. So instead of comparing against pi-cue's own
 ...
 *    (cached in `~/.cache/pi-cue/cued-version.json`, TTL 6h).
...
export function renderWarning(verdict: VersionVerdict): string | null {
  ...
  if (verdict.kind === "unknown-running") {
    lines.push(
      `pi-cue: cued does not report its version; latest cue-shell release is ${verdict.latest}.`,
    );
  } else {
    lines.push(
      `pi-cue: cued ${verdict.daemon.kind === "reported" ? verdict.daemon.version : "(unknown)"} is older than latest cue-shell release ${verdict.latest}.`,
    );
  }
  ...
  try {
    const response = await fetch(url, {
      headers: {
        ...
        "User-Agent": "pi-cue version-check",
      },
