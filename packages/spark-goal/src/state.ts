...
export function cloneGoal(goal: SparkGoal): SparkGoal {
  return {
    ...goal,
...
export function goalsEquivalent(left: SparkGoal, right: SparkGoal): boolean {
  return (
...
export function createSparkGoal(
  objective: string,
  tokenBudget?: number | null,
  now = unixSeconds(),
): SparkGoal {
...
export function runtimeUsageEntry(goal: SparkGoal, at = unixSeconds()): GoalCustomEntry {
...
export function isGoalCustomEntry(data: unknown): data is GoalCustomEntry {
  ...
  return entry.kind === "set" && isSparkGoal(entry.goal);
}
...
export function isSparkGoal(goal: unknown): goal is SparkGoal {
  if (!goal || typeof goal !== "object") {
    return false;
  }
  const candidate = goal as SparkGoal;
  return (
...
export function createGoal(
  ...
  const goal = createSparkGoal(objective, tokenBudget);
  return {
    ok: true,
...
export function updateGoalStatus(current: SparkGoal | null, status: GoalStatus): GoalResult {
  if (!current) {
    return {
      ok: false,
...
): { goal: SparkGoal | null; changed: boolean; crossedBudget: boolean } {
  if (!current) {
