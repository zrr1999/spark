...
Minimal structured ask primitives for Spark extension hosts.

`@zendev-lab/spark-ask` is infrastructure: it does not depend on
`spark-*` packages and focuses on a small, stable protocol
for collecting human input.

## Tools

`@zendev-lab/spark-ask` registers one public/default action tool:
...
option-description validation in the host package; `@zendev-lab/spark-ask` only owns generic
structural validation, reserved UI labels, and ask runtime semantics.

## Behavior comparison and adopted affordances

The local `@zendev-lab/spark-ask` flow is intentionally small, but it now
keeps the useful protocol traits observed in related Pi packages:

- `@eko24ive/spark-ask`: review tab, elaborate/re-ask flow, replay of the
  previous form, preview-aware options, and explicit continuation payloads.
- `spark-ask-user`: split-pane previews, freeform input, comments, and a clear
...
information gate, not just a select menu. Current `@zendev-lab/spark-ask` now adopts the parts
...
1. **Request identity is explicit.** `renderAskScreen()` receives request
   ...
   `packages/spark-ask/src/ui/render.ts` and is covered by
...
likely choices for the user to confirm. `@zendev-lab/spark-ask` supports this with
...
`@zendev-lab/spark-ask` exports shared helpers so callers do not reimplement subtly different
summaries:
