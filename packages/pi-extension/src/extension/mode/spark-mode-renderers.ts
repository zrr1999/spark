...
const RESEARCH_SUBAGENT_STRATEGY =
  'Default lightweight research should use anonymous role calls plus main-agent synthesis when parallel inspection, cross-checking, or specialist review materially improves coverage. Call role({ action: "call", role, instruction }) with focused read-only research briefs; use sessionId instead of role only when persistent conversation continuity is intentional. The main agent remains responsible for summarizing, reconciling, and qualifying the findings.';
