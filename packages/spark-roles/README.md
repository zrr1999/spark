...
Role Markdown loads from project `.agents/roles/**/*.md`, user `~/.agents/roles/**/*.md`, builtins, and loaded extensions. Role files do not carry `model` or `defaultModel`; model settings are stored separately. Resolution order is explicit run model, project settings, then user settings.
