...
Runtime resolves reusable `RoleSpec`s from `@zendev-lab/pi-roles`, calls `runRole()` for concrete child Pi executions, and adapts those `RoleRun`s back into Spark task/run/artifact state. Graph-level ready-task scheduling and durable workflow-run state live in `@zendev-lab/pi-workflows`; `@zendev-lab/spark-runtime` stays focused on one task execution at a time.
